...
@WebServlet("/Pants/*")
public class PantsController extends HttpServlet {
	...
	public void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch (action) {
		case "/list.do": // 중간 네비게이션 pants 페이지 요청
			
			list = pantsService.servicePantsList(request);
			
			center = "pants/pants.jsp";
		...
		case "/priceAsc.do": //낮은 가격순 정렬
			...
			list = pantsService.servicePriceAsc(request);
		...
		case "/pricDesc.do": //높은 가격순 정렬	
			...
			list = pantsService.servicePriceDesc(request);
		...
		case "/searchList.do": //상품명 검색
			...
			list = pantsService.serviceSerachList(request);
			center = "pants/pants.jsp";
		...
		case "/productDetail.do" : //상세페이지에 보여줄 상품 정보
			...
			boolean result = pantsService.serviceWishValue(m_id,p_idx); //위시리스트에 있는지 없는지 판단
			
			center = "pants/productDetail.jsp";
